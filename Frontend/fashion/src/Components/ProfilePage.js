...
function ProfilePage() {
 ...
 const handleEditSubmit = async (e) => {
  ...
  try {
    const storedUser = JSON.parse(localStorage.getItem("user"));
    const uid = storedUser?.UID;
    const res = await axios.post(`http://localhost:5000/api/profile/edit`, {
      uid,
      ...editForm,
    });
    if (res.data.success) {
      ...
      // Fetch updated user after closing modal
      axios
        .get(`http://localhost:5000/api/profile/${uid}`)
        .then((res) => setUser(res.data.user));
    } else {
      setEditError(res.data.error || "Failed to update profile.");
    }
  } catch (err) {
